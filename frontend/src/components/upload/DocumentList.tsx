...
export default function DocumentList({ onDocumentSelect }: DocumentListProps) {
  ...
  if (loading) return <div className="text-[12px] text-muted-foreground/80 animate-pulse mt-8 flex items-center justify-center gap-2">
    <div className="w-3 h-3 rounded-full border-2 border-border border-t-foreground animate-spin"></div>
    Loading library...
  </div>;
  ...
  return (
    <div className="mt-8 flex-1 overflow-y-auto">
        ...
        <div className="mb-4 p-4 border border-rose-200 bg-rose-50/50 border-l-2 border-l-rose-500">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-1 text-[12px] font-semibold px-3 py-2 border border-border bg-card text-foreground hover:bg-muted transition-all"
            >
              Cancel
            </button>
          </div>
      ...
      {documents.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-6 border border-dashed border-border bg-muted">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-6 h-6 text-muted-foreground/50 mb-2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
          <p className="text-[12px] text-muted-foreground font-medium">Empty repository</p>
        </div>
      ) : (
          ...
          {documents.map((doc) => (
            <li key={doc} className="group">
              <button
                ...
                className="w-full text-left flex items-center gap-3 text-[13px] font-medium text-foreground bg-card p-2.5 border border-border hover:border-ring hover:text-foreground hover:bg-muted transition-all cursor-pointer"
              >
                <span className="flex items-center justify-center w-7 h-7 bg-muted border border-border text-muted-foreground group-hover:text-foreground transition-colors shrink-0">
                  {doc.endsWith('.mp4') ? (
